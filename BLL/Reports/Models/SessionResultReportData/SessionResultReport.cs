...
namespace BLL.Reports.Models.SessionResultReportData
{
    ...
    public class SessionResultReport : ISessionResultReport
    {
        ...
        public SessionResultReport(string connectionString = @"Data Source=KONSTANTINPC\SQLEXPRESS; Initial Catalog=ResultSession; Integrated Security=true;") => _groupTable = new GroupTable(connectionString);

        /// <inheritdoc cref="ISessionResultReport.GetReport(int)"/>
        public SessionResultReportView GetReport(int sessionId) => new SessionResultReportView { GroupTables = _groupTable.GetGroupTableData(sessionId) };

        /// <inheritdoc cref="ISessionResultReport.GetReport(int, Func{GroupTableRowView, object}, bool)"/>
        public SessionResultReportView GetReport(int sessionId, Func<GroupTableRowView, object> predicate, bool isDescOrder) => new SessionResultReportView { GroupTables = _groupTable.GetGroupTableData(sessionId, predicate, isDescOrder) };
    }
}